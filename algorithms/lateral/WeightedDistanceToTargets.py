...
from qgis.core import QGis, QgsFeature, QgsGeometry, QgsPoint, QgsSpatialIndex, QgsFields, QgsField, QgsWKBTypes, NULL
...
class QueueEntry(object):
    ...
    def __init__(self, key, parent, weight, score):
        self.key = key
        self.parent = parent
        self.weight = weight
        self.score = score
...
class EdgeData(object):
    ...
    def __init__(self, edge_id, from_node, to_node, weight, score):
...
class EdgeLayerGraph(object):
    ...
    def weight(self, edge):
        ...
        return self.score(edge) * edge.geometry().length()
    ...
    def edges(self, node_key):
        ...
        if self.index.has_key(node_key):
            for edge_data in self.index[node_key]:
                w = edge_data.score
                if self.max_weight > 0 and w < self.max_weight:
                    yield edge_data
...
class UndirectedEdgeLayerGraph(EdgeLayerGraph):
    ...
    def build_index(self):
        ...
        for edge in self.edge_layer.getFeatures():
            ...
            for node in (a, b):
                edge_data = EdgeData(edge.id(), node, other(node, a, b), self.weight(edge), self.score(edge))
...
class DirectedEdgeLayerGraph(EdgeLayerGraph):
    ...
    def build_index(self):
        ...
        for edge in self.edge_layer.getFeatures():
            a = edge.attribute(self.node_a_field)
            b = edge.attribute(self.node_b_field)
            edge_data = EdgeData(edge.id(), a, b, self.weight(edge), self.score(edge))
...
class GraphIterator(object):
    ...
    def __next__(self):
        ...
        for edge_data in self.graph.edges(next_entry.key):
            ...
            score = max(next_entry.score, edge_score)
            ...
            if self.seen.has_key(node):
                ...
                if weight < seen_entry.weight:
                    ...
                    new_entry = QueueEntry(node, next_entry, weight, score)
...
class WeightedDistanceToTargets(GeoAlgorithm):
    ...
    def defineCharacteristics(self):
        ...
        self.name, self.i18n_name = self.trAlgorithm('Weighted Distance To Targets')
