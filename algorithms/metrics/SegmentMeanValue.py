...
class ElevationService(object):
    ...
    def __enter__(self):
        ...
        self.nodata = self.elevations.GetNoDataValue()
        return self
    ...
    def sample_linestring(self, linestring, step):
        ...
        for s in np.arange(0, length+step, step):
            ...
            z = self.point_elevation(p)
            if z is not None:
                yield p.x(), p.y(), z, s

    def project_linestring(self, linestring):
        """ Returns projected linestring
            as a sequence of (x, y, z, m) coordinates
            where m is the 'measure' coordinate,
            ie. curvilinear coordinate of point measured along line
        ...
        if linestring.isMultipart():
            ...
            for points in linestring.asMultiPolyline():
                ...
                for a, b in zip(points[:-1], points[1:]):
                    for x, y, z, m in self.project_segment(a, b):
                        if z is not None:
                            yield x, y, z, m0 + m
    ...
    def project_segment(self, a, b):
        ...
        if dx > 0.0 or dy > 0.0:
            ...
            while i < n:
                ...
                z = self.point_elevation(QgsPoint(x, y))
                if z is not None:
                    yield x, y, z, m
...
class SegmentMeanValue(GeoAlgorithm):
    ...
    def defineCharacteristics(self):
        ...
        self.name, self.i18n_name = self.trAlgorithm('Segment Mean Value')
        ...
        self.addParameter(ParameterRaster(self.INPUT_RASTER,
                                          self.tr('Raster Values')))

        self.addParameter(ParameterString(self.PREFIX,
                                          self.tr('Field Prefix'),
                                          default='VAL_'))
        ...
        self.addOutput(OutputVector(self.OUTPUT, self.tr('Segment Values')))

    def processAlgorithm(self, progress):
        
        line_layer = dataobjects.getObjectFromUri(self.getParameterValue(self.INPUT_LINESTRINGS))
        dem_layer = dataobjects.getObjectFromUri(self.getParameterValue(self.INPUT_RASTER))
        prefix = self.getParameterValue(self.PREFIX)
        ...
        writer = self.getOutputFromName(self.OUTPUT).getVectorWriter(
            vector_helper.createUniqueFieldsList(
                line_layer,
                QgsField(prefix + 'MEAN', QVariant.Double, len=10, prec=6),
                QgsField(prefix + 'MEDIAN', QVariant.Double, len=10, prec=6)
            ),
        ...
        with ElevationService(dem_layer.dataProvider().dataSourceUri(), int(code2), int(code1)) as service:
            ...
            for current, feature in enumerate(vector.features(line_layer)):
                ...
                values = np.array([ z for x, y, z, m in service.project_linestring(feature.geometry()) ])
