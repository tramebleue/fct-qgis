...
def resolve_flat(elevations, flow, feedback=None):
    ...
    elevations: array-like, ndims=2, dtype=float32 or float64
        Elevation raster, preprocessed for depression filling
    ...
    int32 NumPy array, with same shape as `elevations` nodata = 0
    """
    ...
    for i in range(height):
        for j in range(width):
            ...
            for x in range(8):
                ...
                if not ingrid(xi, xj) or flow[xi, xj] == FLOW_NODATA:
                    continue
    ...
    total = 100.0 / (len(low_queue) + len(high_queue))

    for current, (i, j) in enumerate(low_queue + high_queue):
